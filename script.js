...
function getGPS() {
  ...
  var reportObj = {
    id: 'REP-' + Math.floor(1000 + Math.random() * 9000),
    type: type.value,
    desc: (descEl && descEl.value.trim()) || 'Observed ' + type.value.toLowerCase() + ' in the area.',
    lat: (latEl && parseFloat(latEl.value)) || (_activeStation ? _activeStation.latitude : 25.57),
    lon: (lonEl && parseFloat(lonEl.value)) || (_activeStation ? _activeStation.longitude : 91.89),
    station: _activeStation ? _activeStation.name : 'NER Regional Station',
    state: _activeStation ? _activeStation.state : 'Meghalaya',
    time: 'Just now',
    status: 'pending',
    verifiedBy: null,
    verifiedAt: null
  ...
  if (_fieldReports) {
    _fieldReports.unshift(reportObj);
    updateReportCounts();
    renderReportCards();
  }
  ...
  ['vs2', 'vs3', 'vs4'].forEach(function (id) {
    ...
    if (el) {
      el.className = 'verify-step';
      el.textContent = el.id === 'vs2' ? '⏳ Cross-checking rainfall data…' :
                       el.id === 'vs3' ? '⏳ Checking against risk model…' :
  ...
  setTimeout(function () {
    var el = document.getElementById('vs4');
    if (el) { el.className = 'verify-step verify-step--done'; el.textContent = '✅ Logged for authority review'; }
    var vr2 = document.getElementById('verifyResult');
    if (vr2) {
      vr2.style.display = 'block';
      vr2.style.background = '#f0fdf4';
